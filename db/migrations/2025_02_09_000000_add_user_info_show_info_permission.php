...
class AddUserInfoShowInfoPermission extends Migration
{
    ...
    public function up(): void
    {
        ...
        $this->db->table('privileges')
            ->insertOrIgnore([
                'name' => 'user.info.show.info',
                'description' => 'Show hint user info exists',
            ]);
        $userInfoShowInfoId = $this->getPrivilegeId('user.info.show.info');
    ...
    {
        $this->db->table('privileges')
            ->where('name', 'user.info.show.info')
            ->delete();
    }
