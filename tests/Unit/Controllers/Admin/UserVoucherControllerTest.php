...
class UserVoucherControllerTest extends ControllerTest
{
    ...
    public function testSaveVoucherJsonResponce(): void
    {
        $got_voucher = 4;
        $body = ['got_voucher' => $got_voucher];
        $response_body = ['issued' => $got_voucher, 'eligible' => $got_voucher + UserVouchers::eligibleVoucherCount($this->user), 'total' => 4];
        $request = $this->request->withAttribute('user_id', $this->user->id)->withParsedBody($body)->withHeader('accept','application/json', );
