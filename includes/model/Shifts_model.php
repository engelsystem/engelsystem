...
function Shifts_free($start, $end, ShiftsFilter $filter = null)
{
    ...
    $shifts = Db::select('
        ...
        FROM (
            ...
            > (SELECT COUNT(*) FROM `shift_entries` WHERE `shift_entries`.`shift_id`=`shifts`.`id` AND shift_entries.`freeload_user_id` IS NULL' . ($filter ? ' AND shift_entries.angel_type_id IN (' . implode(',', $filter->getTypes()) . ')' : '') . ')
            AND s.shift_id IS NULL
...
function Shift_free_entries(AngelType $needed_angeltype, $shift_entries)
{
    ...
    foreach ($shift_entries as $shift_entry) {
        if (is_null($shift_entry->freeload_user_id)) {
            $taken++;
        }
    }
...
function Shifts_by_user($userId, $include_freeloaded_comments = false)
{
    $shiftsData = Db::select(
        ...
        SELECT
            ...
            `shift_entries`.`freeload_user_id`,
