...
function user_settings() {
  global $enable_tshirt_size, $tshirt_sizes, $themes, $locales, $database;
  global $user;
  ...
  elseif (isset($_REQUEST['submit_database'])) {
    $ok = true;

    if (isset($_REQUEST['dbase']) && isset($database[$_REQUEST['dbase']])){
...
$database = $dbh->query( 'SHOW DATABASES' );
}
  ...
  elseif (isset($_REQUEST['reinstall'])) {
    $ok = true;

      success("System Re-insatlled.");
      redirect(page_link_to('user_settings'));
    }
  ...
  return page_with_title("Admin Settings", array(
      ...
      div('row', array(
          ...
          div('col-md-6', array(
             ...
             form(array(
              	  form_info(_("Here you can choose your Database:")),
              	  form_select('dbase',_("Databases:"), $database, $selected_database),
              	  form_submit('submit_database', _("Save"))
              )),
              form(array(
                  form_info(_("Here you can reinstall the system")),
                  form_submit('reinstall', _("Reinstall"))
              ))
