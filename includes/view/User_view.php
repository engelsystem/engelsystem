...
function User_view_myshift(Shift $shift, $user_source, $its_me, $supporter)
{
    ...
    $myshift = [
        ...
        'duration' => Carbon::formatDuration(CarbonInterval::diff($shift->start, $shift->end)),
        'hints' => $night_shift,
...
function User_view_myshifts(
    ...
    if (count($myshifts_table) > 0) {
        ...
        if ($show_sum) {
            $myshifts_table[] = [
                'date' => '<b>' . __('Sum:') . '</b>',
                'duration' => '<b>' . Carbon::formatDuration(CarbonInterval::seconds((int) $timeSum)) . '</b>',
                'hints' => '',
                'location' => '',
                'shift_info' => '',
                'comment' => '',
...
function User_view_worklog(Worklog $worklog, $admin_user_worklog_privilege, $its_me)
{
    ...
    return [
        ...
        'duration' => Carbon::formatDuration(CarbonInterval::minutes(round($worklog->hours * 60))),
        'hints' => $night_shift,
        'location' => '',
