...
class ShiftTableShiftRenderer extends ShiftCalendarShiftRenderer
{
    ...
    protected function renderRegisterdAngels($shift, $needed_angeltypes, $shift_entries, $user)
    {
        ...
        foreach ($needed_angeltypes as $angeltype) {
            if ($angeltype['count'] > 0 || count($shift_entries_filtered[$angeltype['id']]) > 0) {
                list($angeltype_signup_state, $angeltype_html) = $this->renderShiftRegistedAngelType(
                    $shift,
                    $shift_entries_filtered[$angeltype['id']],
                    $angeltype,
                    $user
    ...
    protected function renderShiftRegistedAngelType($shift, $shift_entries, $angeltype, $user)
    {
        $entry_list = [];
        foreach ($shift_entries as $entry) {
            $class = $entry['freeloaded'] ? 'text-decoration-line-through' : '';
            $entry_list[] = '<span class="text-nowrap ' . $class . '">' . User_Nick_render($entry) . '</span><br/>';
        }
