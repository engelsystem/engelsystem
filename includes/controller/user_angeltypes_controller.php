...
function user_angeltypes_delete_all_controller(): array
{
    ...
    if (!auth()->user()->isAngelTypeSupporter($angeltype) && !auth()->can('user.angeltypes.edit')) {
...
function user_angeltypes_confirm_all_controller(): array
{
    ...
    $angeltype = AngelType::findOrFail($request->input('angeltype_id'));
    if (!auth()->can('user.angeltypes.edit') && !$user->isAngelTypeSupporter($angeltype)) {
...
function user_angeltype_update_controller(): array
{
    ...
    if (!auth()->can('user.angeltypes.edit') && !config('supporters_can_promote')) {
...
function user_angeltype_join_controller(AngelType $angeltype)
{
    ...
    if ($request->hasPostData('submit')) {
        ...
        if (auth()->can('user.angeltypes.edit') && $request->hasPostData('auto_confirm_user')) {
